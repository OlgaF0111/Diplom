...
public class SqlHelper {                                                                            //класс - Помощник по Sql
    public static Connection getConnection() throws SQLException {                                //метод установки соединения с БД.
    ...
    public static void cleanDb() {                                                                //метод очистки базы данных
        ...
        try (val conn = getConnection();                                                          //определяет блок кода, в котором может произойти исключение
            ...
            runner.update(conn, order);                                                           //команда обнови,внеси изменения в строку(объект) ???
    ...
    public static String getStatusPaymentEntity() {                                              //метод получения статуса платежного идентификатора (для дебетовой карты)
        try (val conn = getConnection();                                                         //определяет блок кода, c исключением
            ...
            val sql = "SELECT status FROM payment_entity ORDER BY created DESC LIMIT 1;";        //SQL= запрос на получение статусов, которые есть внутри таблицы payment_entity(платежный идентификатор),при сортировки результирующего набора созданного в порядке возрастания
            val resultSet = countStmt.executeQuery(sql);                                         //получение выполненных запросов=результирующий запрос в базу SQL
